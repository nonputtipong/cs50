...
def index():
    ...
    test = db.execute(
        "SELECT symbol, name, SUM(shares) AS shares, price FROM test WHERE user_id = (?) GROUP BY symbol HAVING SUM(shares) > 0;", user_id)
    ...
    return render_template("index.html", database=test, users=cash, sum=sum)
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("UPDATE users SET cash = (?) WHERE id = (?);",
                   update_user_cash, user_id)
        db.execute("INSERT INTO test (user_id, symbol, name, shares, price) VALUES (?, ?, ?, ?, ?)",
...
@login_required
def history():
    """Show history of test"""
    user_id = session["user_id"]

    test = db.execute(
        "SELECT symbol, name, shares, price, Timestamp FROM test WHERE user_id = (?);",
        user_id,
    )
    ...
    return render_template("history.html", database=test, buy_sell=buy_sell)
...
def sell():
    ...
    if request.method == "GET":
        symbols = db.execute(
            "SELECT symbol FROM test WHERE user_id = (?) GROUP BY symbol HAVING SUM(shares) > 0;", user_id,)
    ...
    old_shares = db.execute(
        "SELECT symbol, SUM(shares) AS shares FROM test WHERE symbol = (?);",
        symbol,
    )
    ...
    db.execute("UPDATE users SET cash = (?) WHERE id = (?);",
               update_user_cash, user_id)
    db.execute(
        "INSERT INTO test (user_id, symbol, name, shares, price) VALUES (?, ?, ?, ?, ?);",
        user_id,
        symbol,
        name,
        shares * (-1),
